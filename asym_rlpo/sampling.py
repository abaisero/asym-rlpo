...
# though)
def sample_episode(env: gym.Env, *, num_steps: int) -> GV_Episode:
    env.reset()
    ...
    for _ in range(num_steps):
        # TODO for now this is random action;  need to inject behavior policy
...
def sample_episodes(
    env: gym.Env, *, num_episodes: int, num_steps: int
) -> List[GV_Episode]:
    return [
        sample_episode(env, num_steps=num_steps) for _ in range(num_episodes)
    ]


def sample_rewards(env: gym.Env, *, num_steps: int) -> List[float]:
    # TODO this is means for evaluation, so we need to inject the behavior
    # policy into the sample_episode methods
    episode = sample_episode(env, num_steps=num_steps)
