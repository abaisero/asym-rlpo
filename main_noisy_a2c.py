...
from asym_rlpo.algorithms import NoisyA2C, make_noisy_a2c_algorithm
...
def parse_args():
    ...
    assert bool(re.fullmatch(r'POMDP-heavenhell_(\d+)-episodic-v(\d+)', args.env))
    assert args.latent_type == 'state'
...
# TODO this can just be a generic model
def save_model(model: NoisyActorCriticModel):
    config = get_config()
    ...
    save_data(config.filename_model, data)
...
def run_training(
    ...
    losses = average_losses(
        [
            runstate.algo.compute_losses(
                ...
                pomdp=runstate.env._env._env.unwrapped,
            )
    ...
    objectives = {
        ...
        'h_critic': losses['h_critic'],
        'hz_critic': losses['hz_critic'],
    }
...
def log_training(runstate: Runstate, training_data: TrainingData):
    ...
    losses_logdata = {
        f'training/losses/{key}': loss for key, loss in training_data.losses.items()
    }
...
# TODO this can just be a generic model
def save_modelseq(timestep: int, model: NoisyActorCriticModel):
    config = get_config()
    data = {
        'metadata': {'config': config._as_dict()},
        'data': {
