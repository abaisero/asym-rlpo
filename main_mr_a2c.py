...
from asym_rlpo.algorithms import MemoryReactive_A2C, make_mr_a2c_algorithm
...
def make_runstate(checkpoint: Checkpoint | None) -> Runstate:
    ...
    policies = RunstatePolicies(
        agent=algo.actor_critic_model.actor_model.policy(),
        memory=algo.actor_critic_model.actor_model.history_model.memory_policy,
    )
...
def run_simulation(
    ...
    runstate.policies.memory.epsilon = runstate.epsilon_schedule(
        runstate.xstats.simulation_timesteps
    )
...
def save_modelseq(timestep: int, model: MemoryReactive_ActorCriticModel):
    ...
    filename = config.filename_modelseq_template.format(timestep)
